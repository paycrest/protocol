-- First, check if the XOF-BEN fiat currency exists
SELECT EXISTS (
    SELECT 1 FROM "fiat_currencies"
    WHERE "code" = 'XOF-BEN'
);

-- If the XOF-BEN fiat currency exists, then add the institutions
DO $$
DECLARE
    fiat_currency_id UUID;
    last_bucket_id BIGINT;
BEGIN
    -- Get the ID of the XOF-BEN fiat currency
    SELECT "id" INTO fiat_currency_institutions
    FROM "fiat_currencies"
    WHERE "code" = 'XOF-BEN';

    -- Add institutions to the XOF-BEN fiat currency
    WITH institutions (code, name, type, updated_at, created_at) AS (
        VALUES
            ('MOMOBJPC', 'MTN Mobile Money', 'mobile_money', now(), now())
    )
