...
// The FinancialInstitutionFunc type is an adapter to allow the use of ordinary
// function as FinancialInstitution mutator.
type FinancialInstitutionFunc func(context.Context, *ent.FinancialInstitutionMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f FinancialInstitutionFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.FinancialInstitutionMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.FinancialInstitutionMutation", m)
}
