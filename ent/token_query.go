...
type TokenQuery struct {
	...
	withSenderOrders      *SenderOrderTokenQuery
	withFKs               bool
...
// QuerySenderOrders chains the current query on the "sender_orders" edge.
func (tq *TokenQuery) QuerySenderOrders() *SenderOrderTokenQuery {
...
func (tq *TokenQuery) Clone() *TokenQuery {
	...
	return &TokenQuery{
		...
		withSenderOrders:      tq.withSenderOrders.Clone(),
...
// WithSenderOrders tells the query-builder to eager-load the nodes that are connected to
// the "sender_orders" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TokenQuery) WithSenderOrders(opts ...func(*SenderOrderTokenQuery)) *TokenQuery {
	query := (&SenderOrderTokenClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withSenderOrders = query
	return tq
}
...
func (tq *TokenQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Token, error) {
	...
	if query := tq.withSenderOrders; query != nil {
		if err := tq.loadSenderOrders(ctx, query, nodes,
			func(n *Token) { n.Edges.SenderOrders = []*SenderOrderToken{} },
			func(n *Token, e *SenderOrderToken) { n.Edges.SenderOrders = append(n.Edges.SenderOrders, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TokenQuery) loadSenderOrders(ctx context.Context, query *SenderOrderTokenQuery, nodes []*Token, init func(*Token), assign func(*Token, *SenderOrderToken)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Token)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.SenderOrderToken(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(token.SenderOrdersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.token_sender_orders
		if fk == nil {
			return fmt.Errorf(`foreign-key "token_sender_orders" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "token_sender_orders" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
